...
def organizar_arquivos(pasta_id, cache_id, main_id, buffer_id, pequenos_id, medios_id, grandes_id, docs_id, sheets_id, forms_id, pdfs_id):
    ...
    for conteudo in lista_conteudos['files']:
        ...
        tamanho_arquivo = obter_tamanho_arquivo(arquivo_id)
        ...
        # Separação de arquivos por tamanho
        if tamanho_arquivo > 1 * 1024 * 1024 * 1024:  # Maior que 1GB
            mover_arquivo(arquivo_id, grandes_id)
        elif 500 * 1024 * 1024 < tamanho_arquivo <= 1 * 1024 * 1024 * 1024:  # Entre 500MB e 1GB
            mover_arquivo(arquivo_id, medios_id)
        else:  # Menor ou igual a 500MB
            mover_arquivo(arquivo_id, pequenos_id)

        # Separação de arquivos por tipo
        if conteudo['mimeType'] == 'application/vnd.google-apps.document':
            mover_arquivo(arquivo_id, docs_id)
        elif conteudo['mimeType'] == 'application/vnd.google-apps.spreadsheet':
            mover_arquivo(arquivo_id, sheets_id)
        elif conteudo['mimeType'] == 'application/vnd.google-apps.form':
            mover_arquivo(arquivo_id, forms_id)
        elif conteudo['mimeType'] == 'application/pdf':
            mover_arquivo(arquivo_id, pdfs_id)
...
def main():
    ...
    pequenos_id = 'pequenos_folder_id'  # ID da pasta para arquivos pequenos
    medios_id = 'medios_folder_id'  # ID da pasta para arquivos médios
    grandes_id = 'grandes_folder_id'  # ID da pasta para arquivos grandes
    docs_id = 'docs_folder_id'  # ID da pasta para arquivos Google Docs
    sheets_id = 'sheets_folder_id'  # ID da pasta para arquivos Google Sheets
    forms_id = 'forms_folder_id'  # ID da pasta para arquivos Google Forms
    pdfs_id = 'pdfs_folder_id'  # ID da pasta para arquivos PDF
    ...
    organizar_arquivos(pasta_raiz_id, cache_id, main_id, buffer_id, pequenos_id, medios_id, grandes_id, docs_id, sheets_id, forms_id, pdfs_id)
